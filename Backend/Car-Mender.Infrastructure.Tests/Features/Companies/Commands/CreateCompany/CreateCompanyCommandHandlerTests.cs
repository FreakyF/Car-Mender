...
public class CreateCompanyCommandHandlerTests
{
	...
	public async Task Handle_ValidCommand_ShouldReturnValidId()
	{
		...
		_mockValidator.Setup(v =>
				v.ValidateAsync(It.Is<CreateCompanyCommand>(cmd => cmd == command), It.IsAny<CancellationToken>()))
			.ReturnsAsync(validationResult);

		_mockMapper.Setup(m => m.Map<Company>(It.Is<CreateCompanyCommand>(cmd => cmd == command)))
			.Returns(company);

		_mockCompanyRepository.Setup(r =>
				r.CreateCompanyAsync(It.Is<Company>(c => c == company), It.IsAny<CancellationToken>()))
